...
void PangolinDSOViewer::run()
{
    ...
    pangolin::Var<bool> settings_BlackBackGround("ui.BlackBG",true,true);
    ...
    pangolin::Var<bool> settings_showImmaturePoints("ui.showImmaturePoints",false,true);
    pangolin::Var<bool> settings_showCurrentPoints("ui.showCurrentPoints",false,true);

    pangolin::Var<int> settings_numGridx("ui.numGridx",6,2,20,false);
    pangolin::Var<int> settings_numGridy("ui.numGridy",4,2,20,false);
    pangolin::Var<int> settings_minPointsInPlane("ui.minPiP",18,4,100,false);
    pangolin::Var<float> settings_EigenvalueReg1("ui.EvReg1",100,1,500,false);
    pangolin::Var<float> settings_EigenvalueReg2("ui.EvReg2",10,1,100,false);

    pangolin::Var<int> settings_depthLevel("ui.depthLevel",-1,-1,4,false);
	...
	while( !pangolin::ShouldQuit() && running )
	{
		...
		if(!settings_BlackBackGround.Get())
        ...
        setting_showDepth=settings_depthLevel.Get();
        setting_showImmaturePoint=settings_showImmaturePoints.Get();
        setting_showCurrentPoint=settings_showCurrentPoints.Get();
        setting_minPointsForClustering=settings_minPointsInPlane.Get();
        setting_minEigenvalueReq=settings_EigenvalueReg1.Get();
        setting_maxEigenvalueReg=settings_EigenvalueReg2.Get();
        setting_numGridx=settings_numGridx.Get();
        setting_numGridy=settings_numGridy.Get();
        ...
        if(settings_pauseButton.Get()){
            ...
            setting_pauseProcess = !setting_pauseProcess;
        }
