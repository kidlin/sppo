...
template<int mode>
void AccumulatedTopHessianSSE::addPlane(EFPlane* p, EnergyFunctional const * const ef, int tid)	// 0 = active, 1 = linearized, 2=marginalize
{
    assert(mode==0 || mode==1 || mode==2);
    ...
    Vec3f bn_acc(0,0,0);
    Mat33f Hdn_acc=Mat33f::Zero();
    MatC3f Hcn_acc=MatC3f::Zero();
    ...
    for(EFPResidual* r : p->residualsAll)
    {
        ...
        PlaneResidualJacobian* rJ = r->J;
        int htIDX = r->hostIDX + r->targetIDX*nframes[tid];
        Mat18f dp = ef->adHTdeltaF[htIDX];

        std::vector<float> resApprox;
        if(mode==0)
            resApprox = rJ->resF;
        if(mode==2)
            resApprox = r->res_toZeroF;
        ...
        for(int i=0;i<r->num;i++)
        {
            ...
            acc[tid][htIDX].update(
                    rJ->Jpdc[0][i].data(), rJ->Jpdxi[0][i].data(),
                    rJ->Jpdc[1][i].data(), rJ->Jpdxi[1][i].data(),
                    rJ->JIdx2[i](0,0),rJ->JIdx2[i](0,1),rJ->JIdx2[i](1,1));

            acc[tid][htIDX].updateBotRight(
                    rJ->Jab2[i](0,0), rJ->Jab2[i](0,1), Jab_r[0],
                    rJ->Jab2[i](1,1), Jab_r[1],rr);

            acc[tid][htIDX].updateTopRight(
                    rJ->Jpdc[0][i].data(), rJ->Jpdxi[0][i].data(),
                    rJ->Jpdc[1][i].data(), rJ->Jpdxi[1][i].data(),
                    rJ->JabJIdx[i](0,0), rJ->JabJIdx[i](0,1),
                    rJ->JabJIdx[i](1,0), rJ->JabJIdx[i](1,1),
                    JI_r[0], JI_r[1]);


            Mat23f Ji2_Jpdn = rJ->JIdx2[i] * rJ->Jpdn[i];
            bn_acc += rJ->Jpdn[i].transpose()*JI_r;
            Hdn_acc += Ji2_Jpdn.transpose()*rJ->Jpdn[i];
            Mat42f JpdC; JpdC.col(0)=rJ->Jpdc[0][i]; JpdC.col(1)=rJ->Jpdc[1][i];
            Hcn_acc += JpdC*Ji2_Jpdn;
...
template void AccumulatedTopHessianSSE::addPlane<0>(EFPlane* p, EnergyFunctional const * const ef, int tid);
template void AccumulatedTopHessianSSE::addPlane<1>(EFPlane* p, EnergyFunctional const * const ef, int tid);
template void AccumulatedTopHessianSSE::addPlane<2>(EFPlane* p, EnergyFunctional const * const ef, int tid);
