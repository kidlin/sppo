...
public:
    ...
    template<int mode> void addPlane(EFPlane* p, EnergyFunctional const * const ef, int tid=0);
    ...
    template<int mode> void addPlanesInternal(
            std::vector<EFPlane*>* planes, EnergyFunctional const * const ef,
            int min=0, int max=1, Vec10* stats=0, int tid=0)
    {
        for(int i=min;i<max;i++) addPlane<mode>((*planes)[i],ef,tid);
    }
