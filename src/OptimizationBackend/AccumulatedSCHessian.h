...
public:
	...
	void addPlane(EFPlane* p, bool shiftPriorToZero, int tid=0);
    ...
    void addPlanesInternal(
            std::vector<EFPlane*>* planes, bool shiftPriorToZero,
            int min=0, int max=1, Vec10* stats=0, int tid=0)
    {
        for(int i=min;i<max;i++) addPlane((*planes)[i],shiftPriorToZero,tid);
    }
