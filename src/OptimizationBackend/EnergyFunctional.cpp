...
void EnergyFunctional::accumulateAF_MT(MatXX &H, VecX &b, bool MT)
{
	if(MT)
	{
        ...
        red->reduce(boost::bind(&AccumulatedTopHessianSSE::addPlanesInternal<0>,
                                accSSE_top_A, &allPlanes, this,  _1, _2, _3, _4), 0, allPlanes.size(), 50);
	...
	else
	{
		...
		for(EFFrame* f : frames){
            ...
            for(EFPlane* p: f->planes)
                accSSE_top_A->addPlane<0>(p,this);
		}
...
void EnergyFunctional::accumulateLF_MT(MatXX &H, VecX &b, bool MT)
{
	if(MT)
	{
        ...
        red->reduce(boost::bind(&AccumulatedTopHessianSSE::addPlanesInternal<1>,
                                accSSE_top_L, &allPlanes, this,  _1, _2, _3, _4), 0, allPlanes.size(), 50);
	...
	else
	{
		...
		for(EFFrame* f : frames){
            ...
            for(EFPlane* p: f->planes)
                accSSE_top_L->addPlane<1>(p,this);
		}
...
void EnergyFunctional::accumulateSCF_MT(MatXX &H, VecX &b, bool MT)
{
	if(MT)
	{
        ...
        red->reduce(boost::bind(&AccumulatedSCHessianSSE::addPlanesInternal,
                                accSSE_bot, &allPlanes, true,  _1, _2, _3, _4), 0, allPlanes.size(), 50);
...
void EnergyFunctional::resubstituteF_MT(VecX x, CalibHessian* HCalib, bool MT)
{
	...
	if(MT){
        ...
        red->reduce(boost::bind(&EnergyFunctional::resubstituteFPl,
                                this, cstep, xAd,  _1, _2, _3, _4), 0, allPlanes.size(), 50);
	}
	else{
        resubstituteFPt(cstep, xAd, 0, allPoints.size(), 0,0);
        resubstituteFPl(cstep, xAd, 0, allPlanes.size(), 0,0);
	}
...
void EnergyFunctional::resubstituteFPl(
            const VecCf &xc, Mat18f* xAd, int min, int max, Vec10* stats, int tid)
{
    for(int k=min;k<max;k++)
    {
        EFPlane* p = allPlanes[k];

        int ngoodres = 0;
        for(EFPResidual* r : p->residualsAll) if(r->isActive()) ngoodres++;
        if(ngoodres==0)
        {
            p->data->step.setZero();
            continue;
        ...
        auto bn = p->bnSumF;
        bn -= (p->Hcn_accAF+p->Hcn_accLF).transpose()*xc;

        for(EFPResidual* r : p->residualsAll)
        {
            if(!r->isActive()) continue;
            bn -= xAd[r->hostIDX*nFrames + r->targetIDX] * r->JpJnF;
        }

        p->data->step = - p->HniF*bn;
        assert(std::isfinite(p->data->step.norm()));
    }
}
...
EFPResidual* EnergyFunctional::insertResidual(PlaneFrameResidual* r)
{
    EFPResidual* efr = new EFPResidual(r, r->plane->efPlane, r->host->efFrame, r->target->efFrame);
    efr->idxInAll = r->plane->efPlane->residualsAll.size();
    r->plane->efPlane->residualsAll.push_back(efr);
...
EFPlane* EnergyFunctional::insertPlane(PlaneHessian *pl)
{
    EFPlane* efp=new EFPlane(pl, pl->host->efFrame);
    efp->idxInPlanes=pl->host->efFrame->planes.size();
    pl->host->efFrame->planes.push_back(efp);
...
void EnergyFunctional::dropResidual(EFPResidual* r)
{
    EFPlane* p = r->plane;
    assert(r == p->residualsAll[r->idxInAll]);
...
void EnergyFunctional::marginalizePlanesF()
{
    assert(EFDeltaValid);
    assert(EFAdjointsValid);
    assert(EFIndicesValid);
    ...
    for(EFFrame* f : frames)
    {
        ...
        {
            EFPlane* p = f->planes[i];
            if(p->stateFlag == EFPlaneStatus::PL_MARGINALIZE)
            {
                p->priorF *= setting_idepthFixPriorMargFac;
                for(EFPResidual* r : p->residualsAll)
                    if(r->isActive())
                        connectivityMap[(((uint64_t)r->host->frameID) << 32) + ((uint64_t)r->target->frameID)][1]++;
                allPlanesToMarg.push_back(p);
            }
        }
    }
    ...
    accSSE_bot->setZero(nFrames);
    accSSE_top_A->setZero(nFrames);
    for(EFPlane* p : allPlanesToMarg)
    {
        accSSE_top_A->addPlane<2>(p,this);
        accSSE_bot->addPlane(p,false);
        removePlane(p);
    }
    MatXX M, Msc;
    VecX Mb, Mbsc;
...
void EnergyFunctional::dropPlanesF()
{
    for(EFFrame* f : frames)
    {
        for(int i=0;i<(int)f->planes.size();i++)
        {
            EFPlane* p = f->planes[i];
            if(p->stateFlag == EFPlaneStatus::PL_DROP)
            {
                removePlane(p);
                i--;
            }
...
void EnergyFunctional::removePlane(EFPlane* p)
{
    for(EFPResidual* r : p->residualsAll)
        dropResidual(r);

    EFFrame* h = p->host;
    h->planes[p->idxInPlanes] = h->planes.back();
    h->planes[p->idxInPlanes]->idxInPlanes = p->idxInPlanes;
    h->planes.pop_back();

    nPlanes--;
    p->data->efPlane = nullptr;
