...
namespace dso
{
EFPResidual::EFPResidual(PlaneFrameResidual* org, EFPlane* plane_, EFFrame* host_, EFFrame* target_)
:data(org), plane(plane_), host(host_), target(target_)
{
        isLinearized=false;
        isActiveAndIsGoodNEW=false;
        num=plane->data->num;
        J = new PlaneResidualJacobian(num);
        res_toZeroF.resize(num);
        JpJnF.setZero();
...
void EFPResidual::takeDataF()
{
    std::swap<PlaneResidualJacobian*>(J, data->J);

    JpJnF.setZero();
    for(int k=0;k<num;k++){
        if(!data->mask[k]) continue;
        Mat23f JI_JI_Jn = J->JIdx2[k] * J->Jpdn[k];
        for(int i=0;i<6;i++)
            for(int j=0;j<3;j++)
                JpJnF(i,j)+=J->Jpdxi[0][k][i]*JI_JI_Jn(0,j)+J->Jpdxi[1][k][i]*JI_JI_Jn(1,j);

        JpJnF.block<2,3>(6,0)+=J->JabJIdx[k]*J->Jpdn[k];
    }
}
...
{
    priorF.setZero();
    deltaF = data->iNormal-data->iNormal_zero;
}
...
void EFPResidual::fixLinearizationF(EnergyFunctional* ef)
{
    Vec8f dp = ef->adHTdeltaF[hostIDX+ef->nFrames*targetIDX];
    ...
    {
        Vec3f Jpdn_x=J->Jpdn[i].row(0);
        Vec3f Jpdn_y=J->Jpdn[i].row(1);

        // compute Jp*delta
        __m128 Jp_delta_x = _mm_set1_ps(J->Jpdxi[0][i].dot(dp.head<6>())
                                        +J->Jpdc[0][i].dot(ef->cDeltaF)
                                        +Jpdn_x.dot(plane->deltaF));
        __m128 Jp_delta_y = _mm_set1_ps(J->Jpdxi[1][i].dot(dp.head<6>())
                                        +J->Jpdc[1][i].dot(ef->cDeltaF)
                                        +Jpdn_y.dot(plane->deltaF));

        // PATTERN: rtz = resF - [JI*Jp Ja]*delta.
        __m128 rtz = _mm_load_ps(((float*)J->resF.data())+i);
        rtz = _mm_sub_ps(rtz,_mm_mul_ps(_mm_load_ps(((float*)(J->JIdx))+i),Jp_delta_x));
        rtz = _mm_sub_ps(rtz,_mm_mul_ps(_mm_load_ps(((float*)(J->JIdx+1))+i),Jp_delta_y));
        rtz = _mm_sub_ps(rtz,_mm_mul_ps(_mm_load_ps(((float*)(J->JabF))+i),delta_a));
        rtz = _mm_sub_ps(rtz,_mm_mul_ps(_mm_load_ps(((float*)(J->JabF+1))+i),delta_b));
        _mm_store_ps(((float*)res_toZeroF.data())+i, rtz);
    }
