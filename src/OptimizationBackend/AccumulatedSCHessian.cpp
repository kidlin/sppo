...
void AccumulatedSCHessianSSE::addPlane(EFPlane* p, bool shiftPriorToZero, int tid)
{
    int ngoodres = 0;
    ...
    if(ngoodres==0)
    {
        p->HniF.setZero();
        p->bnSumF.setZero();
        p->data->iNormal_Hessian.setZero();
        return;
    }

    auto H = p->Hdn_accAF+p->Hdn_accLF+p->priorF;
    assert(H.determinant()>0.1);

    p->data->iNormal_Hessian=H;
    p->HniF = H.inverse();
    assert(isfinite(p->HniF.norm()));
    p->bnSumF = p->bn_accAF + p->bn_accLF;
    if(shiftPriorToZero) p->bnSumF += p->priorF*p->deltaF;
    auto Hcn = p->Hcn_accAF + p->Hcn_accLF;
    accHcc[tid].update(Hcn,Hcn,p->HniF);
    accbc[tid].update(Hcn, p->HniF*p->bnSumF);


    int nFrames2 = nframes[tid]*nframes[tid];
    for(EFPResidual* r1 : p->residualsAll)
    {
        if(!r1->isActive()) continue;
        int r1ht = r1->hostIDX + r1->targetIDX*nframes[tid];

        for(EFPResidual* r2 : p->residualsAll)
        {
            if(!r2->isActive()) continue;
            accD[tid][r1ht+r2->targetIDX*nFrames2].update(r1->JpJnF, r2->JpJnF, p->HniF);
        }

        accE[tid][r1ht].update(r1->JpJnF, Hcn, p->HniF);
        accEB[tid][r1ht].update(r1->JpJnF,p->HniF*p->bnSumF);
    }
}
