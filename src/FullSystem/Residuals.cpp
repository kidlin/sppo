...
int PlaneFrameResidual::instanceCounter = 0;

long runningResID=0;


PointFrameResidual::PointFrameResidual(){assert(false); instanceCounter++;}
PlaneFrameResidual::PlaneFrameResidual(){assert(false); instanceCounter++;}

PointFrameResidual::~PointFrameResidual(){assert(efResidual==0); instanceCounter--; delete J;}
PlaneFrameResidual::~PlaneFrameResidual(){assert(efPResidual==0); instanceCounter--; delete J;}
...
PlaneFrameResidual::PlaneFrameResidual(PlaneHessian* plane_, FrameHessian* host_, FrameHessian* target_)
:plane(plane_),host(host_),target(target_)
{
    efPResidual=0;
    instanceCounter++;
    resetOOB();
    num=plane_->num;
    J = new PlaneResidualJacobian(num);
    centerProjectedTo.resize(num);
    mask=std::vector<bool>(num,true);
    assert(((long)J)%16==0);
...
double PlaneFrameResidual::linearize(CalibHessian* HCalib)
{
    state_NewEnergyWithOutlier=-1;
    ...
    { state_NewState = ResState::OOB; return state_energy; }
        ...
        const Mat33f &PRE_RTll_0 = precalc->PRE_RTll_0;
        const Vec3f &PRE_tTll_0 = precalc->PRE_tTll_0;
        const float * const color = plane->colors.data();
        const float * const weights = plane->weights.data();
        ...
        for(int idx=0;idx<num;idx++)
        {
            ...
            if(!projectPoint(plane->u[idx], plane->v[idx], plane->iNormal_zero_scaled,HCalib,
                             PRE_RTll_0,PRE_tTll_0, drescale, u, v, Ku, Kv, KliP, new_idepth))
            { num_OOB++; mask[idx]=false; continue; }

            centerProjectedTo[idx] = Vec3f(Ku, Kv, new_idepth);
            ...
            d_n(0,0)=d_d_x*(Ku-HCalib->cxl())*HCalib->fxli();
            d_n(0,1)=d_d_x*(Kv-HCalib->cyl())*HCalib->fyli();
            d_n(0,2)=d_d_x;
            d_n(1,0)=d_d_y*(Ku-HCalib->cxl())*HCalib->fxli();
            d_n(1,1)=d_d_y*(Kv-HCalib->cyl())*HCalib->fyli();
            d_n(1,2)=d_d_y;

            J->Jpdxi[0][idx] = d_xi_x;
            J->Jpdxi[1][idx] = d_xi_y;
...

            {
                ...
                J->JIdx2[idx](0,0) = JIdxJIdx_00;
                J->JIdx2[idx](0,1) = JIdxJIdx_10;
                J->JIdx2[idx](1,0) = JIdxJIdx_10;
                J->JIdx2[idx](1,1) = JIdxJIdx_11;
                J->JabJIdx[idx](0,0) = JabJIdx_00;
                J->JabJIdx[idx](0,1) = JabJIdx_01;
                J->JabJIdx[idx](1,0) = JabJIdx_10;
                J->JabJIdx[idx](1,1) = JabJIdx_11;
                J->Jab2[idx](0,0) = JabJab_00;
                J->Jab2[idx](0,1) = JabJab_01;
                J->Jab2[idx](1,0) = JabJab_01;
                J->Jab2[idx](1,1) = JabJab_11;
            }
        }

        state_NewEnergyWithOutlier = energyLeft;

        if(energyLeft > std::max<float>(host->frameEnergyTH, target->frameEnergyTH) || wJI2_sum < 2 || num_OOB*1.5>num)
        {
            energyLeft = std::max<float>(host->frameEnergyTH, target->frameEnergyTH);
            state_NewState = ResState::OUTLIER;
        }
    ...
    void PlaneFrameResidual::debugPlot() {
        //TODO
        return;
    }
...
void PlaneFrameResidual::applyRes(bool copyJacobians)
{
    if(copyJacobians)
    {
         if(state_state == ResState::OOB)
         {
             assert(!efPResidual->isActiveAndIsGoodNEW);
             return;	// can never go back from OOB
         }
         if(state_NewState == ResState::IN)// && )
         {
             efPResidual->isActiveAndIsGoodNEW=true;
             efPResidual->takeDataF();
         }
         else
         {
             efPResidual->isActiveAndIsGoodNEW=false;
         }
    }
