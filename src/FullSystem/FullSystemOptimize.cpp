...
void FullSystem::linearizeAllPlane_Reductor(bool fixLinearization, std::vector<PlaneFrameResidual*>* toRemove, int min, int max, Vec10* stats, int tid)
{
    for(int k=min;k<max;k++)
    {
        PlaneFrameResidual* r = activePlaneResiduals[k];
        (*stats)[0] += r->linearize(&Hcalib);
        ...
        if(fixLinearization)
        {
            ...
            if(r->efPResidual->isActive())
            {
                if(r->isNew)
                {
                    PlaneHessian* p = r->plane;
                    p->numGoodResiduals++;
                }
            }
            else
            {
                toRemove[tid].push_back(activePlaneResiduals[k]);
            }
        }
    }
}
...
void FullSystem::applyResP_Reductor(bool copyJacobians, int min, int max, Vec10* stats, int tid)
{
    for(int k=min;k<max;k++)
        activePlaneResiduals[k]->applyRes(true);
}
...
Vec3 FullSystem::linearizeAll(bool fixLinearization)
{
	...
	std::vector<PlaneFrameResidual*> toRemoveP[NUM_THREADS];
	for(int i=0;i<NUM_THREADS;i++) toRemove[i].clear();
    for(int i=0;i<NUM_THREADS;i++) toRemoveP[i].clear();
	...
	if(multiThreading)
	{
        ...
        treadReduce.reduce(boost::bind(&FullSystem::linearizeAllPlane_Reductor, this, fixLinearization, toRemoveP, _1, _2, _3, _4), 0, activePlaneResiduals.size(), 0);
        lastEnergyP += treadReduce.stats[0];
	}
	else
	{
		...
		lastEnergyP = stats[0];
        Vec10 statsP;
        linearizeAllPlane_Reductor(fixLinearization, toRemoveP, 0,activePlaneResiduals.size(),&statsP,0);
        lastEnergyP += stats[0];
	}
	...
	if(fixLinearization)
	{
        ...
        for(PlaneFrameResidual* r : activePlaneResiduals)
        {
            PlaneHessian* pl = r->plane;
            if(pl->lastResiduals[0].first == r)
                pl->lastResiduals[0].second = r->state_state;
            else if(pl->lastResiduals[1].first == r)
                pl->lastResiduals[1].second = r->state_state;
        }
		...
		for(int i=0;i<NUM_THREADS;i++)
		{
            ...
            for(PlaneFrameResidual* r : toRemoveP[i])
            {
                PlaneHessian* pl = r->plane;

                if(pl->lastResiduals[0].first == r)
                    pl->lastResiduals[0].first=0;
                else if(pl->lastResiduals[1].first == r)
                    pl->lastResiduals[1].first=0;

                for(unsigned int k=0; k<pl->residuals.size();k++)
                    if(pl->residuals[k] == r)
                    {
                        ef->dropResidual(r->efPResidual);
                        deleteOut<PlaneFrameResidual>(pl->residuals,k);
                        nResRemoved++;
                        break;
                    }
            }
...
//	meanStepC += Hcalib.step.norm();
	...
	if(setting_solverMode & SOLVER_MOMENTUM)
	{
		...
		for(FrameHessian* fh : frameHessians)
		{
			...
			for(PlaneHessian* pl: fh->planeHessians)
            {
			    Vec3f step=pl->step+0.5*(pl->step_backup);
			    pl->setINormal(pl->iNormal_backup+step);
			    pl->setINormalZero(pl->iNormal_backup+step);
            }
		}
	}
	else
	{
		...
		for(FrameHessian* fh : frameHessians)
		{
            ...
            for(PlaneHessian* pl: fh->planeHessians)
            {
                pl->setINormal(pl->iNormal_backup+stepfacD*pl->step);
                pl->setINormalZero(pl->iNormal_backup+stepfacD*pl->step);
            }
		}
	}
	...
	if(setting_solverMode & SOLVER_MOMENTUM)
	{
		if(backupLastStep)
		{
			...
			for(FrameHessian* fh : frameHessians)
			{
				...
				for(PlaneHessian* pl:fh->planeHessians)
                {
				    pl->iNormal_backup=pl->iNormal;
				    pl->step_backup=pl->step;
                }
			}
		}
		else
...
void FullSystem::loadSateBackup()
{
	...
	for(FrameHessian* fh : frameHessians)
	{
		...
		for(PlaneHessian* pl:fh->planeHessians)
        {
		    pl->setINormal(pl->iNormal_backup);
		    pl->setINormalZero(pl->iNormal_backup);
        }
...
float FullSystem::optimize(int mnumOptIts)
{
	...
	for(FrameHessian* fh : frameHessians){
        ...
        for(PlaneHessian* pl:fh->planeHessians)
        {
            ...
            {
                if(!r->efPResidual->isLinearized)
                {
                    activePlaneResiduals.push_back(r);
                    r->resetOOB();
                }
            }
        }
	...
	if(multiThreading){
        ...
        treadReduce.reduce(boost::bind(&FullSystem::applyResP_Reductor, this, true, _1, _2, _3, _4), 0, activePlaneResiduals.size(), 50);
    }
	else{
        applyRes_Reductor(true,0,activeResiduals.size(),0,0);
        applyResP_Reductor(true,0,activePlaneResiduals.size(),0,0);
	}
...
void FullSystem::removeOutliers()
{
	...
	for(FrameHessian* fh : frameHessians)
	{
		...
		for(unsigned int i=0;i<fh->planeHessians.size();i++)
        {
		    PlaneHessian* pl = fh->planeHessians[i];
		    if(pl==0) continue;

		    if(pl->residuals.size()==0)
            {
		        fh->planeHessiansOut.push_back(pl);
		        pl->efPlane->stateFlag = EFPlaneStatus::PL_DROP;
		        fh->planeHessians[i]=fh->planeHessians.back();
		        fh->planeHessians.pop_back();
		        i--;
		        numPlanesDropped++;
            }
        }
