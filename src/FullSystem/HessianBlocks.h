...
struct FrameHessian
{
	...
	std::vector<PlaneHessian*> planeHessians;
...
struct PointHessian
{
	...
	static std::vector<Vec3f> idepth2XYZ(const std::vector<PointHessian*>& points, const Mat33f& Ki){
        std::vector<Vec3f> XYZ(points.size());

        for(int i=0;i<points.size();i++){
            XYZ[i]=1/points[i]->idepth*Ki*Vec3f(points[i]->u,points[i]->v,1);
        }

        return XYZ;
    }

    Vec3f idepth2XYZ(const Mat33f& Ki){
        return 1/idepth*Ki*Vec3f(u,v,1);
    }

};

struct PlaneHessian{
    PlaneHessian(FrameHessian* host, Vec3f& iNormal, std::vector<PointHessian*>& points);

    static int instanceCounter;
    EFPlane* efPlane;

    std::vector<PointHessian*> points;
    int num;

    Vec3f eigenvalue;
    Vec3f centroid;
    Vec2f centroid2d;
    Vec2f anchor; // only for showing in the grids

    bool merged;

    // static values
    std::vector<float> colors;              // colors in host frame
    std::vector<float> weights;             // host-weights for respective residuals.
    std::vector<float> u;
    std::vector<float> v;

    // values to optimize
    Vec3f iNormal;
    Vec3f iNormal_scaled;
    Vec3f iNormal_zero_scaled;
    Vec3f iNormal_zero;
    Vec3f step;
    Vec3f step_backup;
    Vec3f iNormal_backup;

    Mat33f iNormal_Hessian;
    int numGoodResiduals;
    ...
    std::vector<PlaneFrameResidual*> residuals;					// only contains good residuals (not OOB and not OUTLIER). Arbitrary order.
    std::pair<PlaneFrameResidual*, ResState> lastResiduals[2]; 	// contains information about residuals to the last two (!) frames. ([0] = latest, [1] = the one before).


    enum PlStatus {ACTIVE=0, INACTIVE, OOB, MARGINALIZED};
    PlStatus status;

    inline void setPlaneStatus(PlStatus s) {status=s;}

    inline void setINormal(Vec3f iNormal) {
        this->iNormal = iNormal;
        this->iNormal_scaled = SCALE_IDEPTH * iNormal;
    }
    inline void setINormalScaled(Vec3f iNormal_scaled) {
        this->iNormal = SCALE_IDEPTH_INVERSE * iNormal_scaled;
        this->iNormal_scaled = iNormal_scaled;
    }
    inline void setINormalZero(Vec3f iNormal) {
        iNormal_zero = iNormal;
        iNormal_zero_scaled = SCALE_IDEPTH * iNormal;
    }

    void updateCC(bool updateFull=false);

    inline float idepthFromPlane(float u, float v, const Mat33f& Ki){
        return (Ki*Vec3f(u,v,1)).dot(iNormal);
    }
    ...
    inline bool isOOB(const std::vector<FrameHessian*>& toKeep, const std::vector<FrameHessian*>& toMarg) const
    {
        ...
        int visInToMarg = 0;
        for(PlaneFrameResidual* r : residuals)
        {
            if(r->state_state != ResState::IN) continue;
            for(FrameHessian* k : toMarg)
                if(r->target == k) visInToMarg++;
