		...
		std::vector<FrameHessian*> frameHessians)
{
	...
	debugShowDepth(setting_showDepth);
...
void CoarseTracker::debugShowDepth(int lvl) {
    if(lvl==-1||lvl>=pyrLevelsUsed) return;

    MinimalImageF depth(w[lvl], h[lvl], idepth[lvl]);
    IOWrap::displayImage("depth",&depth,true);
    //IOWrap::waitKey(10);
}
