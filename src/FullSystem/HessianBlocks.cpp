...
void PlaneHessian::release()
{
    for(auto* p:residuals) delete p;
    residuals.clear();
}
...
void PlaneHessian::updateCC(bool updateFull)
{
    if(!merged){
        anchor[0]=points.front()->u;
        anchor[1]=points.front()->v;
    }

    if(!updateFull) return;
    ...
    float weight_sum=0;
    for(int i=0;i<num;i++){
        colors[i]=points[i]->color[4];
        weights[i]=points[i]->color[4];
        u[i]=points[i]->u;
        v[i]=points[i]->v;
        weight_sum+=weights[i];
    }

    assert(isfinite(weight_sum)&&weight_sum>0);

    for(int i=0;i<num;i++)
        weights[i]/=weight_sum;

    setPlaneStatus(PlStatus::ACTIVE);
}

PlaneHessian::PlaneHessian(FrameHessian* host, Vec3f& iNormal, std::vector<PointHessian*>& points)
:iNormal(iNormal),points(points),host(host)
{
    merged=false;

    instanceCounter++;
    iNormal_Hessian.setZero();

    numGoodResiduals=0;

    num=points.size();

    setPlaneStatus(PlStatus::INACTIVE);
