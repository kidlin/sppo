...
int PlaneHessian::instanceCounter=0;
...
void FullSystem::planarAlign(FrameHessian* fh)
{
    if(fh->planeHessians.empty()&&!fh->flaggedForMarginalization){
        // detect planes at the first time
        fh->planeHessians=planeCluster(fh, fh->pointHessians);
        // smoothe idepth
        Mat33f Ki = Mat33f::Zero();
        Ki(0,0) = Hcalib.fxli();Ki(1,1) = Hcalib.fyli();
        Ki(0,2) = Hcalib.cxli();Ki(1,2) = Hcalib.cyli();Ki(2,2) = 1;
        for(auto* plane:fh->planeHessians){
            for(auto* point:plane->points){
                float idepth=plane->idepthFromPlane(point->u,point->v,Ki);
                point->setIdepth(idepth);
                point->setIdepthZero(idepth);
            }
        }
    }
    else{
        // try to fit points to current planes
        for(auto* plane:fh->planeHessians){
...
void FullSystem::flagPlanesForRemoval()
{
    assert(EFIndicesValid);
    ...
    for(FrameHessian* host : frameHessians)		// go through all active frames
    {
        for(unsigned int i=0;i<host->planeHessians.size();i++)
        {
            PlaneHessian* pl = host->planeHessians[i];
            if(pl==0) continue;

            if(pl->iNormal_Hessian.determinant() < 0.1 || pl->residuals.size()==0)
            {
                host->planeHessiansOut.push_back(pl);
                pl->efPlane->stateFlag = EFPlaneStatus::PL_DROP;
                host->planeHessians[i]=0;
                flag_nores++;
            }
            else if(pl->isOOB(fhsToKeepPoints, fhsToMargPoints) || host->flaggedForMarginalization)
            {
                flag_oob++;
                if(pl->isInlierNew())
                {
                    flag_in++;
                    int ngoodRes=0;
                    for(PlaneFrameResidual* r : pl->residuals)
                    {
                        r->resetOOB();
                        r->linearize(&Hcalib);
                        r->efPResidual->isLinearized = false;
                        r->applyRes(true);
                        if(r->efPResidual->isActive())
                        {
                            r->efPResidual->fixLinearizationF(ef);
                            ngoodRes++;
                        }
                    }
                    if(pl->iNormal_Hessian.determinant() > setting_minINormalH_marg)
                    {
                        flag_inin++;
                        pl->efPlane->stateFlag = EFPlaneStatus::PL_MARGINALIZE;
                            host->planeHessiansMarginalized.push_back(pl);
                    }
                    else
                    {
                        pl->efPlane->stateFlag = EFPlaneStatus::PL_DROP;
        ...
        for(int i=0;i<(int)host->planeHessians.size();i++)
        {
            if(host->planeHessians[i]==0)
            {
                host->planeHessians[i] = host->planeHessians.back();
                host->planeHessians.pop_back();
                i--;
            }
        }
    }
...
void FullSystem::makeKeyFrame( FrameHessian* fh)
{
	...
	for(FrameHessian* fh1 : frameHessians)		// go through all active frames
	{
		...
		for(PlaneHessian* pl: fh1->planeHessians)
        {
		    auto* r=new PlaneFrameResidual(pl,fh1,fh);
		    r->setState(ResState::IN);
		    pl->residuals.push_back(r);
            ef->insertResidual(r);
            pl->lastResiduals[1]=pl->lastResiduals[0];
            pl->lastResiduals[0]=std::pair<PlaneFrameResidual*, ResState>(r,ResState::IN);
        }
	}
    ...
    for(FrameHessian* f : frameHessians)
        if(!f->pointHessians.empty())
            planarAlign(f);
	if(frameHessians.size()>1)
        showCurrentPoint(frameHessians[frameHessians.size()-2]);
...
void FullSystem::initializeFromInitializer(FrameHessian* newFrame)
{
    ...
    int init_time=std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
	printf("INITIALIZE FROM INITIALIZER (%d pts)!\n", (int)firstFrame->pointHessians.size());
    printf("INITIALIZE USED %d ms!\n",init_time);
}
...
void FullSystem::makeNewTraces(FrameHessian* newFrame, float* gtDepth)
{
	...
	int numPointsTotal = pixelSelector->makeMaps(newFrame, selectionMap,setting_desiredImmatureDensity,1,setting_showImmaturePoint);
...
void FullSystem::showCurrentPoint(FrameHessian *host) {
    if(!host) return;
    if(!setting_showCurrentPoint) return;

    MinimalImageB3 img(wG[0],hG[0]);

    for(int i=0;i<wG[0]*hG[0];i++)
    {
        float c = host->dI[i][0]*0.7;
        if(c>255) c=255;
        img.at(i) = Vec3b(c,c,c);
    }

    for(auto* p:host->pointHessians){
        if(!p) continue;

        int u=(int)p->u;
        int v=(int)p->v;
        if(u<1||v<1||u>wG[0]-2||v>hG[0]-2) continue;
        if(isinf(p->idepth)||isnan(p->idepth)) continue;

        img.setPixel9(u,v,Vec3b(10,10,10));
    }

    int M=setting_numGridx,N=setting_numGridy;
    int w=wG[0], h=hG[0];
    int wM=w/M, hN=h/N;
    for(int i=wM;i<w;i+=wM)
        img.setPixelVerLine(i,Vec3b(38,38,38));
    for(int i=hN;i<h;i+=hN)
        img.setPixelHorLine(i,Vec3b(38,38,38));

    int nn=1;
    for(auto* plane:host->planeHessians){
        unsigned char color1=rand()%256;
        unsigned char color2=rand()%256;
        unsigned char color3=rand()%256;
        if(!plane->points.empty()){
            int u=((int)plane->anchor[0]/wM)*wM;
            int v=((int)plane->anchor[1]/hN)*hN;
            img.setPixelRect(u,u+wM,v,v+hN,Vec3b(color1,color2,color3));
        }
        for(auto* p:plane->points){
            ...
            if(u<1||v<1||u>wG[0]-2||v>hG[0]-2) continue;
            if(isinf(p->idepth)||isnan(p->idepth)) continue;
            //img.setPixel9(u,v,Vec3b(color1,color2,color3));
            img.setPixel9(u,v,Vec3b(0,215,255));
        }
        printf("Plane %d (%s) contains %lu points, normal: %f %f %f, eigenvalue: %f %f %f \n",
                nn++,plane->merged?"merged":"single",plane->points.size(),plane->iNormal[0],
                plane->iNormal[1],plane->iNormal[2],plane->eigenvalue[0], plane->eigenvalue[1],
                plane->eigenvalue[2]);
    }

    //printf("num of planes:%lu \n",host->planeHessians.size());

    IOWrap::displayImage("current point",&img,true);

}

std::vector<PlaneHessian*> FullSystem::planeCluster(FrameHessian* host, std::vector<PointHessian*> &points){
    std::vector<PlaneHessian*> planes;

    //we divide the image into M x N grids
    int M=setting_numGridx,N=setting_numGridy;
    int w=wG[0], h=hG[0];
    int wM=w/M, hN=h/N;

    // select points based on the pixel coordinate
    std::vector<std::vector<PointHessian*>> all(M*N,std::vector<PointHessian*>());
    for(auto* p:points){
        int u=int(p->u+0.5f);
        int v=int(p->v+0.5f);
        u/=wM;
    ...
    // plane if the eigen values of the Covariance matrix look good
    Mat33f Ki = Mat33f::Zero();
    Ki(0,0) = Hcalib.fxli();
    Ki(1,1) = Hcalib.fyli();
    Ki(0,2) = Hcalib.cxli();
    ...
    std::vector<std::vector<PlaneHessian*>> label(M,std::vector<PlaneHessian*>(N,nullptr));
    ...
    {
        auto& ps=all[i*N+j];
        if(ps.size()<setting_minPointsForClustering)
            continue;
        std::vector<Vec3f> pointXYZ=PointHessian::idepth2XYZ(ps,Ki);
        Vec3f iNormal, centroid, eigenvalue;
        if(estimateNormal(pointXYZ, centroid, iNormal, eigenvalue)){
            auto* plane= new PlaneHessian(host,iNormal,ps);
            plane->centroid=centroid;
            plane->eigenvalue=eigenvalue;
            plane->num=ps.size();
            plane->updateCC();
            //mask[i][j]=true;

            //try to merge planes with left and top grids
            label[i][j]=plane;
            if(j-1>=0&&label[i][j-1]){
                if(isSimilarPlane(label[i][j-1],plane)){
                    mergePlane(plane,label[i][j-1]);
                    delete label[i][j-1];
                    label[i][j-1]=nullptr;
                    plane->merged=true;
                }
            }
            if(i-1>=0&&label[i-1][j]){
                if(isSimilarPlane(label[i-1][j],plane)){
                    mergePlane(plane,label[i-1][j]);
                    delete label[i-1][j];
                    label[i-1][j]=nullptr;
                    plane->merged=true;
                }
    ...
    for(int i=0;i<M;i++)
        for(int j=0;j<N;j++)
            if(label[i][j]){
                label[i][j]->updateCC(true);
                planes.push_back(label[i][j]);
            }

    // cluster other scattered points
    std::sort(planes.begin(),planes.end(),[](PlaneHessian* p1, PlaneHessian* p2){return p1->num>p2->num;});
    for(auto* p:points){
        if(p->base) continue;

        for(auto* plane:planes){
            if((p->u-plane->centroid2d[0])*(p->u-plane->centroid2d[0])+
                (p->v-plane->centroid2d[1])*(p->v-plane->centroid2d[1])>1.5*(wM*wM+hN*hN)) continue;
            if(fitPlanePoint(plane->iNormal,p->idepth2XYZ(Ki))){
    ...
    for(auto* plane:planes){
        plane->updateCC(true);
        ef->insertPlane(plane);
    }

    //printf("find %lu planes\n",planes.size());

    return planes;
}

bool FullSystem::estimateNormal(std::vector<Vec3f> points, Vec3f& centroid, Vec3f& iNormal, Vec3f& eigenvalue){
    centroid.setZero();
    for(auto& p:points){
        centroid+=p;
    }
    ...
    for(auto& p:points){
        xx+=p[0]*p[0]; xy+=p[0]*p[1]; xz+=p[0]*p[2];
                       yy+=p[1]*p[1]; yz+=p[1]*p[2];
                                      zz+=p[2]*p[2];
    }

    Mat33f H;
    H<<xx,xy,xz,xy,yy,yz,xz,yz,zz;

    Eigen::EigenSolver<Eigen::Matrix3f> es(H);

    Eigen::Matrix3cf evecs = es.eigenvectors();
    Eigen::Vector3cf evals = es.eigenvalues();
    Eigen::Vector3f evalsReal=evals.real();

    std::vector<int> index({0,1,2});
    std::sort(index.begin(),index.end(),[&evalsReal](int i,int j){return evalsReal[i]<evalsReal[j];});

    for(int i=0;i<3;i++)
        eigenvalue[i]=evalsReal[index[i]];

    if(setting_minEigenvalueReq*eigenvalue[0]<eigenvalue[1]&&
        setting_maxEigenvalueReg*eigenvalue[1]>eigenvalue[2])
    {
        auto temp=evecs.real();
        iNormal=temp.col(index[0]);
        float d=iNormal.dot(centroid);
...
bool FullSystem::isSimilarPlane(PlaneHessian* P1, PlaneHessian* P2){
    if(!P1||!P2) return false;
    float d1=1/P1->iNormal.norm();
    float d2=1/P2->iNormal.norm();
    if(abs(d1-d2)>setting_minDisTwoPatches)
        return false;

    float pp=P1->iNormal.dot(P2->iNormal);
    return pp * d1 * d2 > setting_maxCosTwoPatches;
}

void FullSystem::mergePlane(PlaneHessian* P1, PlaneHessian* P2){
    P1->points.insert(P1->points.end(),P2->points.begin(),P2->points.end());
    float w1=(P1->num*1.0f)/(P1->num+P2->num);
    P1->iNormal=w1*P1->iNormal+(1-w1)*P2->iNormal;
    P1->num+=P2->num;
}

bool FullSystem::fitPlanePoint(Vec3f iNormal, Vec3f point){
    float diff=iNormal.dot(point)-1;
    diff=abs(diff)/iNormal.norm();
    if(diff>setting_minDisTwoPatches/2) return false;
    return true;
}
