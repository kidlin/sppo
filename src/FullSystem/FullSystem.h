...
private:
	...
	void flagPlanesForRemoval();
	void makeNewTraces(FrameHessian* newFrame, float* gtDepth);
	void initializeFromInitializer(FrameHessian* newFrame);
	void flagFramesForMarginalization(FrameHessian* newFH);

	void planarAlign(FrameHessian* fh);
	std::vector<PlaneHessian*> planeCluster(FrameHessian* host, std::vector<PointHessian*> &points);
	bool estimateNormal(std::vector<Vec3f> points, Vec3f& centroid, Vec3f& iNormal, Vec3f& eigenvalue);
	static bool isSimilarPlane(PlaneHessian* P1, PlaneHessian* P2);
	static void mergePlane(PlaneHessian* P1, PlaneHessian* P2); //p1 <- p1 + p2
	static bool fitPlanePoint(Vec3f iNormal, Vec3f point);
    ...
    void linearizeAllPlane_Reductor(bool fixLinearization, std::vector<PlaneFrameResidual*>* toRemove, int min, int max, Vec10* stats, int tid);
    ...
    void applyResP_Reductor(bool copyJacobians, int min, int max, Vec10* stats, int tid);
	...
	std::vector<PlaneFrameResidual*> activePlaneResiduals;
 ...
 */
    ...
    static void showCurrentPoint(FrameHessian* host);
};
}
